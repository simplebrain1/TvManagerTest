...
public class UploadUtils {
    ...
    private void initFieldAndCheckFile() throws Exception {
        ...
        if (ServletFileUpload.isMultipartContent(this.request)) {
            ...
            try {
                ...
                errorMsg = "上传文件中有文件的大小超出了单个文件允许的最大值(最大允许：" + fileSizeMax + ")";
                return;
            } catch (FileUploadBase.SizeLimitExceededException sizeLimit) {
                errorMsg = "上传文件大小之和超出允许的最大值(最大允许：" + sizeMax + ")";
                return;
            }
            // 处理请求中的FileItem
    ...
    private void saveFile(FileItem item) throws Exception {
        ...
        if (this.allowRename) {
            ...
            if (TextUtils.isEmpty(this.fileRename) || new File(this.savePath, this.fileRename + fileExt).exists()) {
