...
import com.netease.qa.emmagee.utils.GetFpsUtils;
...
public class EmmageeService extends Service {
	...
	private void startForegroundService(int startId) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			String CHANNEL_ID = "CCOS_MODEL";
			String CHANNEL_NAME = "CCOS_MODEL";
...
//				// get app start time from logcat on every task running
				...
				logAppRunningFrame(packageName);
			} else {
				Intent intent = new Intent();
				intent.putExtra("isServiceStop", true);
				intent.setAction(SERVICE_ACTION);
	...
	private void logAppRunningFrame(String packageName) {
		Log.d(LOG_TAG, "wait flogAppRunningFrame packageName:"+packageName);
...
//				break;
//			}
			GetFpsUtils.getFps("",packageName);
			try {
				Thread.sleep(1000);
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				if (line.matches(regex)) {
					...
					Log.i("getStartTimeFromLogcat", " startTime:"+startTime);
